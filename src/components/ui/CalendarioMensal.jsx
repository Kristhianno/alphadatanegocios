...
//
// Modo seleção de dia (onSelecionarDia definido, usado pela tela de
// agendamento do cliente): a própria célula do dia vira clicável e
// ganha destaque quando é `diaSelecionado`; `desabilitarDia(date)`
// permite cinzentar dias que a regra do vertical já rejeitaria no
// backend (ex: confeitaria exige 24h de antecedência).
    ...
    <div className="bg-surface rounded-card shadow-card p-4">
      ...
      <div className="grid grid-cols-7 gap-px bg-muted-dark border border-muted-dark rounded-input overflow-hidden">
        {dias.map((d) => {
          ...
          const selecionavel = typeof onSelecionarDia === 'function'
          const desabilitado = selecionavel && desabilitarDia?.(d)
          const selecionado = selecionavel && diaSelecionado && chave === formatarChave(diaSelecionado)
          return (
            <div
              key={chave}
              onClick={selecionavel && !desabilitado ? () => onSelecionarDia(d) : undefined}
              className={`min-h-[104px] p-1.5 flex flex-col gap-1 ${doMes ? 'bg-surface' : 'bg-muted/60'} ${
                selecionado ? 'ring-2 ring-inset ring-primary bg-primary-light' : ''
              } ${selecionavel && !desabilitado ? 'cursor-pointer hover:bg-muted' : ''} ${desabilitado ? 'opacity-40 cursor-not-allowed' : ''}`}
            >
              <span
